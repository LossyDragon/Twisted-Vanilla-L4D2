...
ConVar g_hCvarAllow, g_hCvarDecayRate, g_hCvarGlow, g_hCvarGlowCol, g_hCvarGnomeRate, g_hCvarFull, g_hCvarHeal, g_hCvarMaxM, g_hCvarMaxT, g_hCvarMPGameMode, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarRandom, g_hCvarSafe, g_hCvarTemp;
int g_iCvarGlow, g_iCvarGlowCol, g_iCvarFull, g_iCvarHeal, g_iCvarMaxM, g_iCvarRandom, g_iCvarSafe, g_iCvarTemp, g_iGnomeCount, g_iGnome[MAXPLAYERS+1], g_iGnomes[MAX_GNOMES][2], g_iMap, g_iPlayerSpawn, g_iRoundStart;
bool g_bCvarAllow, g_bMapStarted, g_bLoaded;
float g_fCvarMaxT, g_fCvarDecayRate, g_fCvarGnomeRate, g_fHealTime[MAXPLAYERS+1];
...
ConVar g_hCvarIncapHealth, g_hCvarField, g_hCvarFieldRefreshTime, g_hCvarFieldHealAmount, g_hCvarFieldHealAmountIncap, g_hCvarFieldBeacon, g_hCvarFieldColor, g_hCvarFieldStartRadius, g_hCvarFieldEndRadius, g_hCvarFieldDuration, g_hCvarFieldWidth, g_hCvarFieldAmplitude;
int g_iBeamSprite, g_iHaloSprite, g_iCvarIncapHealth, g_iCvarFieldColor[4];
bool g_bCvarField, g_bCvarFieldBeacon, g_bCvarFieldColorRandom;
...
void GetCvars()
{
	...
	g_fCvarMaxT = g_hCvarMaxT.FloatValue + 1.0;
...
void HealClient(int client)
{
	...
	if( g_iCvarTemp == -1 || (g_iCvarTemp != 0 && GetRandomInt(1, 100) >= g_iCvarTemp) )
	{
		...
		if( fHealth + iHealth + fBuff > g_iCvarMaxM )
		{
			if( g_iCvarFull == 1 )
			{
				HealPlayer(client);
				...
				{
					SetEntPropFloat(client, Prop_Send, "m_healthBuffer", g_fCvarMaxT - float(iHealth));
					SetEntPropFloat(client, Prop_Send, "m_healthBufferTime", fGameTime);
				}
			}
		}
	...
	else
	{
		...
		if( fGameTime - g_fHealTime[client] > 1.0 )
		{
			...
			else if( iHealth + fHealth >= g_iCvarMaxM )
			{
				if( fHealth <= g_fCvarMaxT )
				{
					SetEntPropFloat(client, Prop_Send, "m_healthBuffer", g_fCvarMaxT - iHealth);
					SetEntPropFloat(client, Prop_Send, "m_healthBufferTime", fGameTime);
				}
			}
...
void HealClientOnHealingField(int client)
{
	...
	else
	{
		...
		if( g_iCvarTemp == -1 || (g_iCvarTemp != 0 && GetRandomInt(1, 100) >= g_iCvarTemp) )
		{
			...
			if( fHealth + iHealth + fBuff < g_fCvarMaxT )
			{
				SetEntPropFloat(client, Prop_Send, "m_healthBuffer", fHealth + fBuff);
				SetEntPropFloat(client, Prop_Send, "m_healthBufferTime", fGameTime);
			}
		...
		else
		{
			...
			else if( iHealth + fHealth < g_fCvarMaxT )
			{
				SetEntPropFloat(client, Prop_Send, "m_healthBuffer", g_fCvarMaxT - iHealth);
				SetEntPropFloat(client, Prop_Send, "m_healthBufferTime", fGameTime);
			}
