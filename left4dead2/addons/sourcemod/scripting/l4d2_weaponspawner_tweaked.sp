 ...
 * Changelog:
 * v1.4
 *	o - Removed ammo cvars since L4D2 now support them. For M60 support see last version changelog for links.
 * v1.3a
...
public void OnPluginStart()
{
	...
	//M60MaxAmmo = CreateConVar("sm_spawnweapon_m60ammo", "100", "How much Ammo for the M60.", 0, true, 0.0, true, 30.0);
...
public Action Command_SpawnWeapon(int client, int args)
{
	...
	if (StrEqual(weapon, "rifle", false) || StrEqual(weapon, "rifle_ak47", false) || StrEqual(weapon, "rifle_desert", false) || StrEqual(weapon, "rifle_sg552", false))
	{
		maxammo = FindConVar("ammo_assaultrifle_max").IntValue;
	}
	else if (StrEqual(weapon, "weapon_smg", false) || StrEqual(weapon, "weapon_smg_silenced", false) || StrEqual(weapon, "weapon_smg_mp5", false))
	{
		maxammo = FindConVar("ammo_smg_max").IntValue;
	}		
	else if (StrEqual(weapon, "pumpshotgun", false) || StrEqual(weapon, "shotgun_chrome", false))
	{
		maxammo = FindConVar("ammo_shotgun_max").IntValue;
	}
	else if (StrEqual(weapon, "autoshotgun", false) || StrEqual(weapon, "shotgun_spas", false))
	{
		maxammo = FindConVar("ammo_autoshotgun_max").IntValue;
	}
	else if (StrEqual(weapon, "hunting_rifle", false) || StrEqual(weapon, "sniper_military", false))
	{
		maxammo = FindConVar("ammo_huntingrifle_max").IntValue;
	}	
	else if (StrEqual(weapon, "sniper_scout", false) || StrContains(weapon, "sniper_awp", false))
	{
		maxammo = FindConVar("ammo_sniperrifle_max").IntValue;
	}
	else if (StrEqual(weapon, "grenade_launcher", false))
	{
		maxammo = FindConVar("ammo_grenadelauncher_max").IntValue;
	}
	else if (StrEqual(weapon, "rifle_m60", false))
	{
		maxammo = FindConVar("ammo_m60_max").IntValue;
	}
...
public int MenuHandler_SpawnWeapon(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			else if (StrEqual(weapon, "weapon_hunting_rifle", false) || StrEqual(weapon, "weapon_sniper_military", false))
			{
				maxammo = FindConVar("ammo_huntingrifle_max").IntValue;
			}
			else if  (StrEqual(weapon, "weapon_sniper_awp", false) || StrEqual(weapon, "weapon_sniper_scout", false))
			{
				maxammo = FindConVar("ammo_sniperrifle_max").IntValue;
			}
