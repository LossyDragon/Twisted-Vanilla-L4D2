...
ConVar hEnableReloadClipCvar, hEnableClipRecoverCvar, hSmgTimeCvar, hRifleTimeCvar, hHuntingRifleTimeCvar,
	...
	hSniperMilitaryTimeCvar, hGrenadeTimeCvar, hSG552TimeCvar, hM60TimeCvar, hAWPTimeCvar, hScoutTimeCvar, hMagnumTimeCvar;
ConVar hSmgClipCvar, hRifleClipCvar, hHuntingRifleClipCvar, hPistolClipCvar, hDualPistolClipCvar, hSmgSilencedClipCvar,
	...
	hM60ClipCvar, hAWPClipCvar, hScoutClipCvar, hMagnumClipCvar;
...
public void OnPluginStart()
{
	...
	hSG552TimeCvar			= CreateConVar("l4d2_sg552_reload_clip_time", 			"1.3",  "reload time for sg552 clip" 			  , FCVAR_NOTIFY, true, 0.0);
	...
	hMagnumTimeCvar			= CreateConVar("l4d2_magnum_reload_clip_time", 			"1.18", "reload time for magnum clip"  			  , FCVAR_NOTIFY, true, 0.0);
	...
	hM60ClipCvar			= CreateConVar("l4d2_m60_clip", 						"100", 	"m60 max clip"							  , FCVAR_NOTIFY, true, 1.0);
	...
	hMagnumClipCvar			= CreateConVar("l4d2_magnum_clip", 						"8", 	"magnum max clip"				  		  , FCVAR_NOTIFY, true, 1.0);
	...
	hMagnumTimeCvar.AddChangeHook(ConVarChange_CvarChanged);
	...
	hMagnumClipCvar.AddChangeHook(ConVarChange_MaxClipChanged);
	...
	if (IsClientInGame(client) && !IsFakeClient(client) && GetClientTeam(client) == 2 && IsPlayerAlive(client) && buttons & IN_RELOAD) //If survivor alive player is holding weapon and wants to reload
	{
		...
		if(GetEntProp(iCurrentWeapon, Prop_Send, "m_bInReload") == 0)
		{
			...
			switch(weaponid)
			{
				...
				ID_AK47,ID_RIFLE_DESERT,ID_AWP,ID_GRENADE,ID_SCOUT,ID_SG552,ID_M60,
				ID_SNIPER_MILITARY:
				{
...
public Action OnWeaponReload_Event(Event event, const char[] name, bool dontBroadcast) 
{
	...
	switch(weaponid)
	{
		...
		{
			if(IsIncapacitated(client))
				CreateDataTimer(g_MagnumTimeCvar * PISTOL_RELOAD_INCAP_MULTIPLY, WeaponReloadClip, pack, TIMER_FLAG_NO_MAPCHANGE);
			else
				CreateDataTimer(g_MagnumTimeCvar, WeaponReloadClip, pack, TIMER_FLAG_NO_MAPCHANGE);
		}
...
void GetCvars()
{
	...
	g_MagnumTimeCvar		= hMagnumTimeCvar.FloatValue;
}
...
public void SetWeapon()
{
	...
	WeaponAmmoOffest[ID_SNIPER_MILITARY] = 9;
	WeaponAmmoOffest[ID_GRENADE] = 17;
	WeaponAmmoOffest[ID_SG552] = 3;
	WeaponAmmoOffest[ID_M60] = 24;
...
public void SetWeaponMaxClip()
{
	...
	WeaponMaxClip[ID_MAGNUM] = hMagnumClipCvar.IntValue;
