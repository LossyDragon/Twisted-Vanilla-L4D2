...
static ConVar g_hCvar_AbilityThrow;
static ConVar g_hCvar_Limit;
static ConVar g_hCvar_Wait;
...
static bool   g_bCvar_AbilityThrow;
static bool   g_bCvar_Limit;
static bool   g_bCvar_Wait;

// ====================================================================================================
// int - Plugin Variables
// ====================================================================================================
static int    g_iTankClass;
static int    g_iCvar_Limit;

// ====================================================================================================
// float - Plugin Variables
// ====================================================================================================
static float  g_fCvar_Wait;

// ====================================================================================================
// client - Plugin Variables
// ====================================================================================================
static bool   gc_bOnTakeDamagePostHooked[MAXPLAYERS+1];
static bool   gc_bGLExplosiveAmmo[MAXPLAYERS+1];
static int    gc_iStaggerCount[MAXPLAYERS+1];
static float  gc_fLastStagger[MAXPLAYERS+1];

// ====================================================================================================
// entity - Plugin Variables
// ====================================================================================================
static bool   ge_bGLExplosiveAmmo[MAXENTITIES+1];
...
public void Event_PlayerShoved(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (classname[0] == 'w' && StrEqual(classname, "weapon_melee"))
    {
        if (config[CONFIG_ENABLE] == 0)
            return;
...
{
    gc_bOnTakeDamagePostHooked[client] = false;
    gc_bGLExplosiveAmmo[client] = false;
    gc_iStaggerCount[client] = 0;
    gc_fLastStagger[client] = 0.0;
}
...
public void OnClientPutInServer(int client)
{
    ...
    if (gc_bOnTakeDamagePostHooked[client])
        return;

    gc_bOnTakeDamagePostHooked[client] = true;
...
public Action CmdPrintCvars(int client, int args)
{
    ...
    PrintToConsole(client, "l4d_stagger_tank_limit : %i (%s)", g_iCvar_Limit, g_bCvar_Limit ? "true" : "false");
    PrintToConsole(client, "l4d_stagger_tank_wait : %.1f (%s)", g_fCvar_Wait, g_bCvar_Wait ? "true" : "false");
    PrintToConsole(client, "");
    PrintToConsole(client, "======================================================================");
    PrintToConsole(client, "");
