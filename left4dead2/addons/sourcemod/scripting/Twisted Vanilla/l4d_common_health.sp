...
public void SpawnPost(int entity)
{
	...
	if( g_bLeft4Dead2 )
	{
		...
		if( strcmp(model[16], MODEL_CEDA) == 0 )			SetEntProp(entity, Prop_Data, "m_iHealth", g_iCvarHealthCeda);
		else if( strcmp(model[16], MODEL_CLOWN) == 0 )		SetEntProp(entity, Prop_Data, "m_iHealth", g_iCvarHealthClown);
		else if( strcmp(model[16], MODEL_FALLEN) == 0 )		SetEntProp(entity, Prop_Data, "m_iHealth", g_iCvarHealthFallen);
		else if( strcmp(model[16], MODEL_JIMMY) == 0 )		SetEntProp(entity, Prop_Data, "m_iHealth", g_iCvarHealthJimmy);
		else if( strcmp(model[16], MODEL_MUD) == 0 )		SetEntProp(entity, Prop_Data, "m_iHealth", g_iCvarHealthMud);
		else if( strcmp(model[16], MODEL_RIOT) == 0 )		SetEntProp(entity, Prop_Data, "m_iHealth", g_iCvarHealthRiot);
		else if( strcmp(model[16], MODEL_ROAD) == 0 )		SetEntProp(entity, Prop_Data, "m_iHealth", g_iCvarHealthRoad);
	}
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	if( inflictor > MaxClients && inflictor < 2048 && GetGameTime() > g_fLastHit[victim][inflictor] )
	{
		...
		else if( (g_fCvarPipebomb || g_fCvarPropane || g_fCvarOxygen) && strcmp(classname, "pipe_bomb_projectile") == 0 )
		{
			// METHOD 1:
