...
public void OnPluginStart()
{
	...
	if (g_isSequel)
	{
		...
		Scene_L4D1Mourn = CreateConVar(cmd_str, "1", desc_str, FCVAR_ARCHIVE, true, 0.0, true, 1.0);
	}
	else
	{
...
void player_hurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (!IsValidClient(client) || !IsSurvivor(attacker) || attacker == client) return;
...
void GetSurvivorSceneName(int client, bool is_victim = false, char[] str, int maxlength)
{
	...
	else if (strncmp(cl_model, "models/survivors/survivor_biker.mdl", 35, false))			is_victim ? strcopy(str, maxlength, "francis") :	strcopy(str, maxlength, "Biker");
	else if (strncmp(cl_model, "models/survivors/survivor_manager.mdl", 37, false))			is_victim ? strcopy(str, maxlength, "louis") :		strcopy(str, maxlength, "Manager");
