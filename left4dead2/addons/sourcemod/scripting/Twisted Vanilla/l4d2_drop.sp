...
public Plugin myinfo =
{
	...
	author = "Machine, dcx2, Electr000999 /z, Senip, Shao, RainyDagger",
...
public void OnPluginStart()
{
	...
	HookEvent("player_transitioned", Event_MapStart, EventHookMode_PostNoCopy);
...
{
	if(!DoneAlready && !DisableDrop)
	{
		CreateTimer(1.0, AnotherTimer);
		DisableDrop = true;
		DoneAlready = true;
	}
}
...
public Action EnableDrop(Handle Timer)
{
	DisableDrop = false;
}

public Action AnotherTimer(Handle Timer)
{
	if(AllConnected())
	{
		CreateTimer(10.0, EnableDrop);
	}
	else
	{
		CreateTimer(1.0, AnotherTimer);
