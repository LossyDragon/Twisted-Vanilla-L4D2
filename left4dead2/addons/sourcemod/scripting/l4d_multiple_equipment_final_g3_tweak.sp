...
public void OnPluginStart()
{
	...
	if(L4D2Version)
	{
		...
		HookEvent("upgrade_incendiary_ammo", eSpecialAmmo, EventHookMode_Pre);
		HookEvent("upgrade_explosive_ammo", eSpecialAmmo, EventHookMode_Pre);
	}
...
public Action eSpecialAmmo(Handle event, const char[] name, bool dontBroadcast)
{
	if(Ammo == 1 && Clip == 0)
...
void SetClientWeaponInfo_l4d2(int client, int ent , int ammo, int clip, int upgradeBit, int upammo)
{
	if (ent > 0 && GetClientTeam(client) == 2)
	{
		...
		else if (StrEqual(sWeapon, "weapon_hunting_rifle") || StrEqual(sWeapon, "weapon_sniper_military"))
			SetEntData(client, ammoOffset+(36), ammo);
		else if (StrEqual(sWeapon, "weapon_sniper_scout") || StrEqual(sWeapon, "weapon_sniper_awp"))
