				...
				if( sBuff[0] )
				{
					if( g_bLeft4Dead2 )
					{
						if( strcmp(sBuff, "random") == 0 )
						{
							...
							else if( slot >= 10 && slot <= 16 )		g_iPresets[preset][slot] = GetRandomInt(19, 29);	// Random medical/grenade
						} else {
						...
						if( strcmp(sBuff, "random") == 0 )
						{
							...
							else if( slot >= 11 && slot <= 16 )		g_iPresets[preset][slot] = GetRandomInt(6, 10);		// Random medical/grenade
						} else {
							Format(sBuff, sizeof(sBuff), "weapon_%s", sBuff);
...
public void Event_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	if( g_iPlayerSpawn == 1 && g_iRoundStart == 0 )
		CreateTimer(3.0, TimerStart, _, TIMER_FLAG_NO_MAPCHANGE);
	g_iRoundStart = 1;
}
...
void CreateSpawn(const float vOrigin[3], const float vAngles[3], int index, int preset)
{
	...
	for( int slot = 10; slot < 16; slot++ )
		iItem[slot-10] = g_iPresets[preset][slot];
	...
	if( iCountIte && g_iCvarMaxItem )
	{
		...
		{
			dex = iAmItem[x];
			CreateWeapon(iSpawnIndex, dex + 10, iItem[dex] -1, vOrigin, vAngles);
		}
	}
	...
	switch( slot )
	{
		...
		case 10:
		{
			...
			} else if( g_bLeft4Dead2 && strcmp("defibrillator", g_sWeapons2[model][7]) == 0 )
			{
				...
				vPos[2] += 57.0; // 54.0 +3
				MoveForward(vPos, vAng, vPos, -10.0);
				MoveSideway(vPos, vAng, vPos, 13.0); // 11 +2
				vAng[0] += 95.0; // Was 190.0
				vAng[1] -= 90.0;
				vAng[2] -= 90.0;
			} else if( strcmp("pain_pills", g_sWeapons2[model][7]) == 0 )
			{
				vPos = vOrigin;
				vAng = vAngles;
				vPos[2] += 55.0; //54.0 +1
				MoveForward(vPos, vAng, vPos, -10.0);
				MoveSideway(vPos, vAng, vPos, 15.3); //11.0 + 4.3
				vAng[1] += 180.0;
			} else if( g_bLeft4Dead2 && strcmp("adrenaline", g_sWeapons2[model][7]) == 0 )
			{
				vPos = vOrigin;
				vAng = vAngles;
				vPos[2] += 56.0; // 54.0 +2
				MoveForward(vPos, vAng, vPos, -10.0); 
				MoveSideway(vPos, vAng, vPos, 14.0); //11.0 +3
				vAng[1] += 180.0;
			} else{
				vPos = vOrigin;
				vAng = vAngles;
		...
		case 12:
		{
			...
			} else if( strcmp("pain_pills", g_sWeapons2[model][7]) == 0 )
			{
				...
				vPos[2] += 55.0;
				MoveForward(vPos, vAng, vPos, -10.0);
				MoveSideway(vPos, vAng, vPos, 0.3);
				vAng[1] += 180.0;
			} else if(g_bLeft4Dead2 && strcmp("adrenaline", g_sWeapons2[model][7]) == 0)
			{
				vPos = vOrigin;
