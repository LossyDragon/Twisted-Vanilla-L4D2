...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_meteort", cmd_meteor, ADMFLAG_ROOT, "spawn meteor tank"); //1
    RegAdminCmd("sm_shockt", cmd_shock, ADMFLAG_ROOT, "spawn shock tank"); //2
    RegAdminCmd("sm_spawnt", cmd_spawn, ADMFLAG_ROOT, "spawn spawnner tank");  //3
    RegAdminCmd("sm_spittert", cmd_spitter, ADMFLAG_ROOT, "spawn spitter tank"); //4
    RegAdminCmd("sm_healt", cmd_heal, ADMFLAG_ROOT, "spawn heal tank"); //5
    RegAdminCmd("sm_witcht", cmd_witch, ADMFLAG_ROOT, "spawn witch tank"); //6
    RegAdminCmd("sm_firet", cmd_fire, ADMFLAG_ROOT, "spawn fire tank"); //7
    RegAdminCmd("sm_icet", cmd_ice, ADMFLAG_ROOT, "spawn ice tank"); //8
    RegAdminCmd("sm_smashert", cmd_smasher, ADMFLAG_ROOT, "spawn smaher tank"); //9
    RegAdminCmd("sm_gravityt", cmd_gravity, ADMFLAG_ROOT, "spawn gravity tank"); //10
    RegAdminCmd("sm_warpt", cmd_warp, ADMFLAG_ROOT, "spawn warp tank"); //11
    RegAdminCmd("sm_shieldt", cmd_shield, ADMFLAG_ROOT, "spawn shield tank"); //12
    RegAdminCmd("sm_jockeyt", cmd_jockey, ADMFLAG_ROOT, "spawn jockey tank"); //13
    RegAdminCmd("sm_cobalt", cmd_cobalt, ADMFLAG_ROOT, "spawn cobalt tank"); //14
    RegAdminCmd("sm_jumpert", cmd_jumper, ADMFLAG_ROOT, "spawn jumper tank"); //15
    RegAdminCmd("sm_ghostt", cmd_ghost, ADMFLAG_ROOT, "spawn ghost tank"); //16
	...
	if (bSuperTanksEnabled)
	{
		if (client > 0 && IsClientInGame(client))
		{
			...
			if (IsTank(client))
			{
				...
				if( bL4DTwo )
				ShowParticle(client, PARTICLE_DEATH, 5.0);
				LittleFlower(Pos, 0);
				LittleFlower(Pos, 1);
			}	
		...
		if (IsTank(client))
		{
			...
			if (index >= 0 && index <= 16)
			{
				if (index != 0 || (index == 0 && bDefaultOverride))
				{
					...
					CPrintToChatAll("[{orange}SUPER TANKS{default}]{red}%s{olive}Appeared/Spawned\n{red} Type {default}: {olive}%d,{red} Speed {default}: {olive}%1.f,{red} Health {default}: {olive}%i", TankName, index, speed, health);
				}
			}
		}
	}
		...
		for (int i=1; i<=MaxClients; i++)
		{
			if (IsClientInGame(i))
			{
				...
				else if (GetClientTeam(i) == 3)
				{
					...
					if (zombie == 8)
					{
						CreateTimer(0.1, TankLifeCheck, i, TIMER_FLAG_NO_MAPCHANGE);
					}
				}
			}
		}
...
public Action cmd_meteor(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
		if (IsTank(client))
		{
			int index = GetSuperTankByRenderColor(GetEntityRender_RGB(client));
			if (index >= 0 && index <= 16)
			{
				if (index != 0 || (index == 0 && bDefaultOverride))
				{
					...
					FakeClientCommand(client, "z_spawn_old %s auto", TankName);
				}
			}
		}
	}
...
public Action cmd_shock(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
...
public Action cmd_spitter(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
...
public Action cmd_fire(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
...
public Action cmd_smasher(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
...
public Action cmd_warp(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
...
public Action cmd_shield(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
...
public Action cmd_jockey(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
...
public Action cmd_cobalt(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
...
public Action cmd_jumper(int client, int args)
{
    int giveflags = GetCommandFlags("z_spawn_old");
    SetCommandFlags("z_spawn_old", giveflags & ~FCVAR_CHEAT);
	{
