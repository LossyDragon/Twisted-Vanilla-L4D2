...
// ====================================================================================================
static bool   gc_bCharging[MAXPLAYERS+1];
static bool   gc_bStaggered[MAXPLAYERS+1][MAXPLAYERS+1];
...
public void OnPluginStart()
{
    ...
    g_hCvar_Chance     = CreateConVar("l4d2_charge_stagger_chance", "100.0", "Chance to stagger nearby clients while charging.", CVAR_FLAGS, true, 0.1, true, 100.0);
...
public Action TimerStagger(Handle timer)
{
    static float clientPos[3];
    static float targetPos[3];
    static int targetTeamFlag;
    ...
    for (int client = 1; client <= MaxClients; client++)
    {
        ...
        for (int target = 1; target <= MaxClients; target++)
        {
            ...
            if (g_fCvar_Chance < GetRandomFloat(0.1, 100.0))
                continue;

            if (!IsPlayerAlive(target))
                continue;

            targetTeamFlag = GetTeamFlag(GetClientTeam(target));

            if (!(targetTeamFlag & g_iCvar_Team))
                continue;

            switch (targetTeamFlag)
            {
            ...
            if (GetVectorDistance(targetPos, clientPos) > g_fCvar_Distance)
                continue;

            gc_bStaggered[client][target] = true;
            L4D_StaggerPlayer(target, client, clientPos);
        }
    }
}
...
public Action CmdPrintCvars(int client, int args)
{
    ...
    PrintToConsole(client, "l4d2_charge_stagger_chance : %.1f%%", g_fCvar_Chance);
...
bool IsPlayerImmobilized(int client)
{
    ...
    if (g_bL4D2)
    {
        if (GetEntProp(client, Prop_Send, "m_jockeyAttacker") > 0)
            return true;

        if (GetEntProp(client, Prop_Send, "m_pummelAttacker") > 0)
            return true;

        if (GetEntProp(client, Prop_Send, "m_carryAttacker") > 0)
            return true;
    }
...
bool IsPlayerImmobilizing(int client)
{
    ...
    if (g_bL4D2)
    {
        if (GetEntProp(client, Prop_Send, "m_jockeyVictim") > 0)
            return true;
