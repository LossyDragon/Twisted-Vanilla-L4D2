...
static const	M60_OFFSET_IAMMO 				= 24;
...
public OnPluginStart()
{
	...
	//HRAmmoCVAR = CreateConVar("l4d2_guncontrol_huntingrifleammo", "150", " How much Ammo for the Hunting Rifle and Military Sniper Rifle ", DEFAULT_FLAGS);
	//SniperRifleAmmoCVAR = CreateConVar("l4d2_guncontrol_sniperrifleammo", "180", " How much Ammo for the AWP, and Scout ", DEFAULT_FLAGS);	
	...
	//M60AmmoCVAR = CreateConVar("l4d2_guncontrol_m60ammo", "100", " How much Ammo for the M60 ", DEFAULT_FLAGS);	
	...
	GLtoM60TransformCVAR = CreateConVar("l4d2_guncontrol_turnGLintoM60chance", "0", " Turns GL spawns into M60 spawns. Works as chance setting. 1 is FULL chance, 2 is half chance, 3 one third and so on ", DEFAULT_FLAGS);
...
//	SetConVarInt(FindConVar("ammo_m60_max"), GetConVarInt(M60AmmoCVAR));
//}
	...
	while ((ent = FindEntityByClassname(ent, "weapon_grenade_launcher_spawn")) != -1)
	{
		if (prev)
		{
			if (GetRandomInt(1, chance) == 1)
			{
				...
				SetEntProp(replacement, Prop_Data, "m_iExtraPrimaryAmmo", FindConVar("ammo_m60_max").IntValue, 1);
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	if (buttons & IN_USE && !buttondelay[client])
	{
		...
		{ //case: Assault rifles
			...
			maxammo = FindConVar("ammo_assaultrifle_max").IntValue; //get max ammo as set
		}
		...
		{ //case: SMGS
			...
			maxammo = FindConVar("ammo_smg_max").IntValue; //get max ammo as set
		}		
		...
		{ //case: Pump Shotguns
			...
			maxammo = FindConVar("ammo_shotgun_max").IntValue; //get max ammo as set
		}
		...
		{ //case: Auto Shotguns
			...
			maxammo = FindConVar("ammo_autoshotgun_max").IntValue; //get max ammo as set
		}
		else if (StrEqual(ent_name, "weapon_sniper_military", false) || StrEqual(ent_name, "weapon_hunting_rifle", false))
		{ //case: Hunting Rifle / Military Sniper Rifle 
			offsettoadd = HUNTING_RIFLE_OFFSET_IAMMO; //gun type specific offset
			maxammo = FindConVar("ammo_huntingrifle_max").IntValue; //get max ammo as set
		}
		else if (StrEqual(ent_name, "weapon_sniper_awp", false) || StrEqual(ent_name, "weapon_sniper_scout", false))
		{ //case: CSS Snipers
			offsettoadd = MILITARY_SNIPER_OFFSET_IAMMO; //gun type specific offset
			maxammo = FindConVar("ammo_sniperrifle_max").IntValue; //get max ammo as set
		}
		else if (StrEqual(ent_name, "weapon_grenade_launcher", false))
		{ //case: Grenade Launcher
			offsettoadd = GRENADE_LAUNCHER_OFFSET_IAMMO; //gun type specific offset
			maxammo = FindConVar("ammo_grenadelauncher_max").IntValue; //get max ammo as set
		}
		else if (StrEqual(ent_name, "weapon_rifle_m60", false))
		{ //case: M60 Rifle
			offsettoadd = M60_OFFSET_IAMMO; //gun type specific offset
			maxammo = FindConVar("ammo_m60_max").IntValue; //get max ammo as set
		}
		else
		...
		if ((currentammo + foundgunammo) <= maxammo) //if contained ammo is less than youd need to be full
		{
			...
			PrintHintText(client, "You scavenged %i bullets off that gun", foundgunammo); //inform the client
		}
...
public Action:Cmd_GiveAmmo(client, args)
{
	...
	else if (StrEqual(currentgunname, "weapon_sniper_military", false) || StrEqual(currentgunname, "weapon_hunting_rifle", false))
	{ //case: Hunting Rifle / Military Sniper Rifle 
		offsettoadd = HUNTING_RIFLE_OFFSET_IAMMO; //gun type specific offset
		maxammo = FindConVar("ammo_huntingrifle_max").IntValue; //get max ammo as set
	}
	else if (StrEqual(currentgunname, "weapon_sniper_awp", false) || StrEqual(currentgunname, "weapon_sniper_scout", false))
	{ //case: CSS Snipers
		offsettoadd = MILITARY_SNIPER_OFFSET_IAMMO; //gun type specific offset
		maxammo = FindConVar("ammo_sniperrifle_max").IntValue; //get max ammo as set
	}
	else if (StrEqual(currentgunname, "weapon_grenade_launcher", false))
	{ //case: Grenade Launcher
		offsettoadd = GRENADE_LAUNCHER_OFFSET_IAMMO; //gun type specific offset
		maxammo = FindConVar("ammo_grenadelauncher_max").IntValue; //get max ammo as set
	}
