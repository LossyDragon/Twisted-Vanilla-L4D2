...
public OnPluginStart()
{
	...
	CreateConVar("l4d_1gunspawns_version", PLUGIN_VERSION, "Version of the '[L4D & L4D2] Single Guns Spawns' plugin", FCVAR_NOTIFY|FCVAR_SPONLY|FCVAR_DONTRECORD);
	h_CvarGunTypes = CreateConVar("l4d_1gunspawns_types", "15", "Sum of gun types to get replaced (1: Tier 2, 2: Tier 1, 4: Magnum, 8: Pistol, 15: All)", FCVAR_NOTIFY, true, 0.0, true, 15.0);
	h_CvarGunCount = CreateConVar("l4d_1gunspawns_count", "1", "Amount of guns to replace a weapon spawn with", FCVAR_NOTIFY, true, 1.0, true, 16.0);
	h_CvarMaxBotHalt = CreateConVar("l4d_1gunspawns_maxbothalt", "20", "Maximum time (in seconds) the survivor bots will be halted on round start", FCVAR_NOTIFY, true, 0.0, true, 900.0);
	h_CvarMaxClientsLoading = CreateConVar("l4d_1gunspawns_maxloading", "3", "Maximum number of loading clients to ignore on bot reactivation", FCVAR_NOTIFY, true, 0.0, true, 32.0);
	h_CvarShowMessage = CreateConVar("l4d_1gunspawns_message", "0", "Disable/Enable hint message while use is blocked", FCVAR_NOTIFY, true, 0.0, true, 1.0);
